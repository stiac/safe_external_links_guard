...
La versione 1.10.0 introduce una modalità debug integrata che aiuta a diagnosticare i problemi senza inquinare la console quando non serve.
...
- Se vuoi più dettagli attiva anche `settings.debugLevel = 'verbose'` o `data-debug-level="verbose"`. Il livello `basic` mostra gli eventi principali (configurazione, fallback attivati, tracking, errori), mentre `verbose` aggiunge metadati completi, contesto lingua e payload anonimizzati.
...
Nel log compariranno la configurazione normalizzata, la lingua rilevata, i fallback attivati (cache, endpoint non raggiungibile, ecc.), gli eventi di tracking (ID applicati, pixel inviati) e gli errori del resolver. In qualsiasi momento puoi esportare i dati per analisi esterne:
